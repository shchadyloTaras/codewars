...
function topSecret(str) {
  function iterationChars(glyph, beginDecimal, endDecimal) {
    let iterLetters = glyph;
    for (let i = beginDecimal; i < endDecimal; i++) {
      iterLetters.push(String.fromCharCode(i));
    }
    return iterLetters;
  }

  const dict = [];
  iterationChars(dict, 97, 123);

  const DICT = [];
  iterationChars(DICT, 65, 91);
 

  return str.replace(/[a-zA-Z]/gi, v => {
    if (v === v.toLowerCase()) {
